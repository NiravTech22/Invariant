...
from ..core.outcome import Decision, SafetyOutcome
...
class TelemetryBridge:
    ...
    def log_decision(self, state: SystemState, action: ProposedAction, outcome: SafetyOutcome):
        ...
        payload = {
            ...
            "violations": [
                {"rule_id": v.rule_id, "description": v.description, "severity": v.severity}
        ...
        try:
            ...
            requests.post(f"{self.endpoint}/api/v1/telemetry/decision", json=payload, timeout=0.1)
        except Exception:
...
class ConsoleTelemetryBridge(TelemetryBridge):
    def log_decision(self, state, action, outcome):
        ...
        tag = (
            ...
            if outcome.decision == Decision.MODIFIED
            else "[CRIT]"
        )
        print(
            f"[TELEM] {tag} {outcome.decision.value.upper()} | {outcome.reason} ({outcome.processing_time_ms:.2f}ms)"
        )
